...
!******************************************************************************
       ...
       INTEGER,PARAMETER :: NCONFMAX=10000, NCOLMAX=10, NPOINTSMAX=800, NTEMPMAX=100
       REAL*16,PARAMETER :: kB=8.61734E-5, tolprob=1.0E-12, tolminspec=1.0E-6
       INTEGER :: m, auxm, Mm, g, ncoldata, npoints, col, tt, Ntt, nsubs, point
       INTEGER :: mEmin, mEmax
       REAL*16  :: Emin, Emax, paux, maxspec
       REAL*16,DIMENSION(NTEMPMAX) :: Z,E,F,S,T
       REAL*16 :: Zinf, Einf, Sinf
       REAL*16,DIMENSION(NCONFMAX) :: ene, Erel 
       REAL*16,DIMENSION(NCONFMAX,NTEMPMAX) :: p
       REAL*16,DIMENSION(NCONFMAX) :: pinf
       INTEGER,DIMENSION(NCONFMAX) :: conf,omega
       REAL*16,DIMENSION(NCOLMAX,NCONFMAX) :: data 
       REAL*16,DIMENSION(NCOLMAX,NTEMPMAX) :: avedata 
       REAL*16,DIMENSION(NPOINTSMAX,NCONFMAX) :: spec 
       REAL*16,DIMENSION(NPOINTSMAX,NTEMPMAX) :: avespec 
       REAL*16,DIMENSION(NCOLMAX) :: avedatainf 
       REAL*16,DIMENSION(NPOINTSMAX) :: xspec, avespecinf 
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
  ...
  10       continue
    ...
    if (SPECTRA_exists) then
       WRITE(fmtemplist,'(a1,i1,a15)') "(",Ntt,adjustl("(f10.1,2x),a12)")
       write(23,fmtemplist)  T(1:Ntt),"    Infinity"
    endif
  ...
  200          format(f10.1,2x,2(f14.4,2x),e12.6)
    ...
    if (DATA_exists) then
       write(22,201)  T(tt), avedata(1:ncoldata,tt) 
  201          format(f10.1,2x,10(f10.4,2x))
   ...
   300         format(a10,2x,f14.4,8x,a3,7x,e12.6)
    ...
    if (DATA_exists) then
       avedatainf(1:ncoldata)=0.0
       do m=1,Mm
          do col=1,ncoldata
             avedatainf(col)=avedatainf(col)+data(col,m)*pinf(m)
          enddo
       enddo
       write(22,301) adjustr("Infinite"), avedatainf(1:ncoldata) 
